...
namespace Anito.Data.Schema
{
    ...
    public class TypeTable: List<TypeColumn>, IXmlSerializable
    {
        ...
        private string TypeNamespace { get; set; }
        private string TypeNamespaceAssembly { get; set; }
        ...
        public TypeTable(string typeNamespace, string typeNamespaceAssembly)
        {
            TypeNamespace = typeNamespace;
            TypeNamespaceAssembly = typeNamespaceAssembly;
        }
        ...
        public TypeTable(Type type)
        {
            ...
            foreach (var info in minfoArray.Where(i => i.GetCustomAttributes(typeof(Association), true).Length > 0))
            {
                ...
                if(Equals(relation, null))
                    throw new Exception(string.Format("Unknown Relation on type {0}", type.FullName));
                
                //Add Columns
                //Attribute Based Source Column is separated by comma
                var sourceColumns = association.SourceKey.Split(',');
                foreach(var col in sourceColumns)
                {
                    var sourceKeyColumn = this.FirstOrDefault(x => x.Name == col.Trim());    
                    if(Equals(sourceKeyColumn, null)) continue;
                    relation.SourceColumns.Add(sourceKeyColumn);
                }
        ...
        void IXmlSerializable.ReadXml(XmlReader reader)
        {
            if (reader.NodeType == XmlNodeType.Element)
            {
                ...
                var type = ResolveType(assembly);
                if (type == null) throw new Exception(string.Format("Unknown Type {0}", TypeName));
        ...
        private Type ResolveType(string assembly)
        {
            if (!String.IsNullOrEmpty(TypeNamespace))
            {
                TypeName = String.Format("{0}.{1}", TypeNamespace, TypeName); 
            }

            if (!String.IsNullOrEmpty(TypeNamespaceAssembly))
            {
                assembly = TypeNamespaceAssembly;
            }

            return Type.GetType(string.Format("{0}, {1}", TypeName, assembly), false);
        }
